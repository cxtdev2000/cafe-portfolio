...
// Pokes only react while the café is in overview, so close-ups and the intro stay calm.
...
export function Pokeable({ onPoke, bubble, bubbleOffset = [0, 0.4, 0], position, rotation, children }: PokeableProps) {
  ...
  const handleClick = (event: ThreeEvent<MouseEvent>) => {
    ...
    if (!enabled || event.delta > DRAG_THRESHOLD) return;
...
/** One-shot shower of glyph sprites (hearts, sparkles, drops) that plays after each poke. */
