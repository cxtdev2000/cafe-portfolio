...
const CafeScene = dynamic(() => import("./scene/cafe-scene").then((mod) => mod.CafeScene), { ssr: false });
...
export function CafeExperience() {
  ...
  return (
    <main className="h-dvh w-full overflow-hidden bg-espresso">
      <CafeScene view={view} onSelect={setFocus} onReady={() => setReady(true)} />
      ...
      {started && focus === null && (
        <p className="pointer-events-none fixed inset-x-0 bottom-16 z-10 animate-fade-in px-4 md:bottom-6 text-center text-sm text-cream/70 [animation-delay:2s]">
          Kéo để xoay · Chạm vào mọi thứ: nựng mèo, tưới cây, bật đèn…
        </p>
      )}
      {focus === "tv" && (
        <button
          ...
          className="fixed inset-x-0 bottom-5 z-10 mx-auto w-fit animate-fade-in rounded-full bg-espresso/60 px-4 py-2 text-sm text-cream/80 backdrop-blur transition hover:text-cream"
        >
          ← Quay lại
        </button>
      )}
