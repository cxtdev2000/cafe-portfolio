...
const buttonClass =
  "flex h-8 w-8 items-center justify-center rounded-full text-cream/80 transition hover:bg-cream/15 hover:text-cream disabled:opacity-40";

/** Compact background-music pill: previous, play/pause, next. The track itself plays on the café TV. */
export function MusicControls({ visible }: { visible: boolean }) {
  const { ready, playing } = useMusicState();

  return (
    <div
      className={`fixed bottom-5 left-5 z-10 flex items-center gap-0.5 rounded-full bg-espresso/60 p-1 backdrop-blur transition-opacity delay-700 duration-700 ${
